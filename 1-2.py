...
APP_VERSION = "1.0.1"  # phiên bản hiện tại

def check_for_update():
    try:
        resp = requests.get("https://yourdomain.com/update_config.json", timeout=5)
        data = resp.json()
...
def download_update(url, version_str):
    try:
        local_filename = f"AutoRB_{version_str}.exe"
        ...
        print(f"Tải xong: {local_filename}")
        # Khởi động bản mới và thoát chương trình hiện tại
        os.startfile(local_filename)
        sys.exit()
    except Exception as e:
        print(f"Lỗi khi tải file cập nhật: {e}")
